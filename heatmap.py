...
brazil_df = pd.read_csv("results.csv")
...
# Análise de Dados
title = "Oportunidades de Vagas de {Parametro} no Brasil"
col = "Razão"
...
ax.set_title(title, fontdict={'fontsize': '20', 'fontweight': '3'}) #Título
ax.annotate(source, xy=(0.1, .08), xycoords='figure fraction', horizontalalignment='left', verticalalignment='bottom', fontsize=10) #Anotação
sm = plt.cm.ScalarMappable(norm=plt.Normalize(vmin=vmin, vmax=vmax), cmap=cmap).extend('min') #Barra de Cor
